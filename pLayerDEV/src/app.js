...
let app = new Vue({
  ...
  <b-container style="background-color:#E1F3F6;">
    ...
    <hr>
    <div ref="pLayer"></div>
    <b-sidebar v-if="signedIn" id="sidebar-group" title="groups" align="center" shadow backdrop no-header-close>
      <b-col align="center">
  ...
  methods: {
    ...
    async toggleTrack(forward) {
      ...
      if(forward) { this.trackIdx++; }
        ...
        if(!this.trackIdx) this.trackIdx = Object.keys(this.groupTracks).length;
        this.trackIdx--;
      }
      this.trackIdx = this.trackIdx % Object.keys(this.groupTracks).length;
      this.activeTrack = Object.keys(this.groupTracks)[this.trackIdx];
      await this.getTrack();
      this.busy = false;
    },
    async postTrack() {
